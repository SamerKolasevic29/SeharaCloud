...
public class MusicService : IMusicService
{
    ...
    public Task<IEnumerable<MusicDto>> GetAllAsync()
        => _repo.GetAllAsync();

    public Task<IEnumerable<MusicDto>> GetRecentAsync()
        => _repo.GetRecentAsync(RecentLimit);

    public Task<IEnumerable<ArtistDto>> GetArtistsAsync()
        => _repo.GetArtistsAsync();
    ...
    public async Task<IEnumerable<MusicDto>> GetByArtistIdAsync(Guid artistId)
    {
        ...
        // in that cases returns an empty list, not a 404, frontend will hande empty list

        return songs;
    }

    public Task<IEnumerable<GenreDto>> GetGenresAsync()
        => _repo.GetGenresAsync();

    public Task<IEnumerable<MusicDto>> GetByGenreIdAsync(Guid genreId)
        => _repo.GetByGenreIdAsync(genreId);
    ...
    public async Task<IEnumerable<ArtistDto>> SearchArtistsAsync(string query)
    {
        ...
        return await _repo.SearchArtistsAsync(query.Trim());
    }
}